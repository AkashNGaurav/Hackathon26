...
import { TrendingUp, TrendingDown, Award, Briefcase, Globe, Info, Home, GraduationCap, Target } from "lucide-react";
...
const DEFAULT_MFS = ["VFIAX", "VTSAX", "FXAIX", "FUSEX", "PRFDX"];
...
export default function MutualFundPage() {
  ...
  const { mfOfTheDay, topGainers, topLosers } = useMemo(() => {
    ...
    return {
      mfOfTheDay: sorted[0], // Highest positive change
      topGainers: gainers,
      topLosers: losers,
    };
  }, [mfs]);
  ...
  return (
    <div className="p-8 max-w-[90rem] mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[#101410] dark:text-[#f6f3ea]">Mutual Fund Dashboard</h1>
        <p className="mt-2 text-sm text-[#5c6457] dark:text-[#b4ad9f]">
          Comprehensive overview of Mutual Funds, fund metrics, and goal planning.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 rounded-2xl border border-black/10 bg-gradient-to-br from-[#2f6b4f]/10 to-[#2f6b4f]/5 p-6 backdrop-blur-sm dark:border-white/10 dark:from-[#a7d48f]/10 dark:to-[#a7d48f]/5">
            ...
            <div className="mt-4 flex items-baseline gap-2">
              ...
              <span className="text-xs text-gray-500">{mfOfTheDay.currency}</span>
            </div>
          </div>
